...
//try to connect to our database
    $conn = new PDO("mysql:host=$host; dbname=flexchange", $user, $pass);
...
if (isset($_POST['sub_status'])){
    ...
    if ($_POST['status'] == 'flex'){
        echo "flex!";
        $prep = $conn->prepare("UPDATE `user` SET `status` = 0 WHERE `user`.`user_id` = 1;");
        $prep->execute();
    }
    //if user is going offline
    if ($_POST['status'] == 'offline'){
        echo "offline.";
        $prep = $conn->prepare("UPDATE `user` SET `status` = 1 WHERE `user`.`user_id` = 1;");
        $prep->execute();
    }
}
...
if (isset($_POST['sub_location'])){
    ...
    if (isset($_POST['location']) && in_array($_POST['location'], $locations)){
        ...
        $prep = $conn->prepare("UPDATE `user` SET `location` = '$current_location' WHERE `user`.`user_id` = 1;");
        $prep->execute();
...
if (isset($_POST['sub_exchangerate'])){
    ...
    if (isset($_POST['exchangerate'])){
        ...
        $prep = $conn->prepare("UPDATE `user` SET `exchange_rate` = '$exchange_rate' WHERE `user`.`user_id` = 1;");
        $prep->execute();
    }
}
